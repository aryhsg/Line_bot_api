...
4.  **轉傳至 n8n**：本專案接收到 Line 訊息的 JSON Payload 後，會立即將該內容以 **POST 請求** 的方式轉傳（或稱代理）至指定的 `N8N_WEBHOOK_URL`。
...
| 變數名稱 | 說明 | 範例/用途 |
| :--- | :--- | :--- |
| `LINE_CHANNEL_ACCESS_TOKEN` | 您的 Line Bot 存取權杖，用於發送回覆訊息。 | `xxx-your-token-xxx` |
| `LINE_CHANNEL_SECRET` | 您的 Line Bot Channel Secret，用於驗證訊息來源。 | `yyy-your-secret-yyy` |
| `N8N_WEBHOOK_URL` | n8n 工作流程中 **Webhook 節點** 的完整 URL。 | `https://your.n8n.domain/webhook/abcdefg` |
